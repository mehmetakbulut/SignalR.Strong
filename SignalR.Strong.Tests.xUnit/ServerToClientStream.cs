...
namespace SignalR.Strong.Tests.xUnit
{
    public class ServerToClientStream : IClassFixture<ServerFixture>
    {
        ...
        [Fact]
        public async Task RxWithoutTokenExpr()
        {
            var client = await fixture.GetClient();
            var ehub = client.GetExpressiveHub<IMockHub>();

            var data = new List<int>() {1, 2, 3};
            var reader = await ehub.StreamAsChannelAsync(hub => hub.StreamToClient(data));
        ...
        [Fact]
        public async Task RxWithTokenExpr()
        {
            var client = await fixture.GetClient();
            var ehub = client.GetExpressiveHub<IMockHub>();
            ...
            var data = new List<int>() {1, 2, 3};
            var cts = new CancellationTokenSource();
            var reader = await ehub.StreamAsChannelAsync(hub => hub.StreamToClientWithToken(data, cts.Token));
